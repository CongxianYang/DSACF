...
class RGBTDataset(Dataset):
    def __init__(self, seq_dataset, z_r_transforms, z_t_transforms,\
        ...
        self.name          = name
        # self.anchors       = generate_anchors( cfg.total_stride,
    ...
    def open(self):
        ...
        if np.random.rand(1) < cfg.GRAY_RATIO: # why?
        ...
        instance_img_r, _, _, w_x, h_x, _ = self.get_instance_image(
                                                                    ...
                                                                    cfg.instance_size,# 255
                                                                    cfg.context_amount,           # 0.5
                                                                    a_x, b_y,
                                                                    img_mean_rgb_det)

        size_x = cfg.instance_size
        ...
        s_x_t = s_z_t / (cfg.instance_size // 2)
        a_x_t = a_x_ * s_x_t
        b_y_t = b_y_ * s_x_t
    ...
    def crop_and_pad(self, img, cx, cy, gt_w, gt_h, a_x, b_y, model_sz, original_sz, img_mean=None):
        ...
        if not np.array_equal(model_sz, original_sz):
            ...
            if h < w:
                scale_h_ = 1
                scale_w_ = h / w
                scale = cfg.instance_size / h
            elif h > w:
                scale_h_ = w / h
                scale_w_ = 1
                scale = cfg.instance_size / w
            elif h == w:
                scale_h_ = 1
                scale_w_ = 1
                scale = cfg.instance_size / w
    ...
    def compute_target(self, anchors, box):
        ...
        pos_index = np.where(iou > cfg.pos_threshold)[0]
        neg_index = np.where(iou < cfg.neg_threshold)[0]
    ...
    #def __len__(self):
    #  return cfg.pairs_num_per_epoch