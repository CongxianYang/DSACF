...
class GTOT(object):
    ...
    def __getitem__(self, index):
        ...
        img_files_rgb = sorted(glob.glob(os.path.join(
            self.seq_dirs_rgb[index], '*.png*')))
        img_files_t = sorted(glob.glob(os.path.join(
            self.seq_dirs_t[index], '*.png*')))
        ##
        if len(img_files_rgb)==0:
            img_files_rgb = sorted(glob.glob(os.path.join(
                self.seq_dirs_rgb[index], '*.bmp*')))
        if len(img_files_t) ==0:
            img_files_t = sorted(glob.glob(os.path.join(
...
if __name__ == "__main__":
    root_dir = '/home/xiancong/Data_set/GTOT'
