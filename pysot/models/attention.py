...
# 是一种轻量级注意力模块的提出于2018年。CBAM包含CAM（Channel Attention Module）和SAM（Spartial Attention Module）两个子模块
# ，分别在通道上和空间上添加注意力机制。这样不仅可以节约参数和计算力，而且保证了其能够做为即插即用的模块集成到现有的网络架构中去。
# 可以无缝的集成到CNNs中，并且可以与基本CNNs一起端到端的训练。
