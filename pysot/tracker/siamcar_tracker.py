...
class SiamCARTracker(SiameseTracker):
    ...
    def init(self, rgb_img,t_img,bbox,bbox_r,bbox_t):
        """
        args:
        ...
        self.size_r = np.array([bbox_r[2], bbox_r[3]])
        self.center_pos_t = np.array([bbox_t[0] + (bbox_t[2] - 1) / 2,
                                      bbox_t[1] + (bbox_t[3] - 1) / 2])
        self.size_t = np.array([bbox_t[2], bbox_t[3]])
        # calculate z crop size
        #rgb
        w_z_r = self.size_r[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size_r)
        h_z_r= self.size_r[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size_r)
        s_z_r= round(np.sqrt(w_z_r * h_z_r))
        ##T
        w_z_t = self.size_t[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size_t)
        h_z_t = self.size_t[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size_t)
        s_z_t = round(np.sqrt(w_z_t * h_z_t))
        # calculate channle average
        self.channel_average_r = np.mean(rgb_img, axis=(0, 1))
        self.channel_average_t = np.mean(t_img, axis=(0, 1))
        # get crop
        z_crop_r = self.get_subwindow(rgb_img, self.center_pos_r,
                                    cfg.TRACK.EXEMPLAR_SIZE,
                                    s_z_r, self.channel_average_r)
        z_crop_t = self.get_subwindow(t_img, self.center_pos_t,
                                      cfg.TRACK.EXEMPLAR_SIZE,
                                      s_z_t, self.channel_average_t)
        self.model.template(z_crop_r,z_crop_t)
    ...
    def track(self, rgb_img,t_img, hp):
        """
        args:
            img(np.ndarray): BGR image
        return:
        ...
        penalty = np.exp(-(r_c * s_c - 1) * hp['penalty_k'])##w,h bi  cheng fa
        lr = penalty * cls_up[max_r_up, max_c_up] * hp['lr']##计算插值学习率lr=预测框的尺度、宽高比惩罚得分x预测目标点的分类得分x0.3
        ##平滑处理，即跟踪框的变化不要太过于剧烈。于是进行平滑处理后得到了最终的预测框的w、h
        new_width = lr*ave_w + (1-lr)*self.size[0]
        new_height = lr*ave_h + (1-lr)*self.size[1]

        # clip boundary
        cx = bbox_clip(new_cx,0,rgb_img.shape[1])
        cy = bbox_clip(new_cy,0,rgb_img.shape[0])
        width = bbox_clip(new_width,0,rgb_img.shape[1])
        height = bbox_clip(new_height,0,rgb_img.shape[0])
